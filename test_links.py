...
class TestLinksEndpoints:
    ...
    def test_get_links_list(self, client, sample_links):
    ...
    def test_get_link_not_found(self, client, sample_links):
    ...
    def test_create_link(self, client, db):
        ...
        # Verify in database
        get_response = client.get("/links/100")
        assert get_response.status_code == status.HTTP_200_OK
        assert get_response.json()["imdbId"] == "1234567"

    def test_create_link_duplicate(self, client, sample_links):
    ...
    def test_update_link(self, client, sample_links):
        ...
        get_response = client.get("/links/1")
        assert get_response.json()["imdbId"] == "9999999"

    def test_update_link_not_found(self, client, sample_links):
        ...
        response = client.put("/links/9999", json=updated_data)
        
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.json()["detail"] == "Link not found"

    def test_delete_link(self, client, sample_links):
    ...
    def test_delete_link_not_found(self, client, sample_links):
